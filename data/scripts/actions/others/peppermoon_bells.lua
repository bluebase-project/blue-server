...
function peppermoonBells.onUse(player, item, fromPosition, target, toPosition, isHotkey)
	if item.itemid == 23461 then
		player:say("This flower has no pollen left. It will take a little before it has new pollen.", TALKTYPE_POKEMON_SAY, false, player, item:getPosition())
		return true
	else
	if math.random(5) == 1 then
		player:say("Achoo! You spill the pollen into your bag.", TALKTYPE_POKEMON_SAY, false, player, item:getPosition())
		player:addCondition(drunk)
		item:getPosition():sendMagicEffect(CONST_ME_STUN)
	else
		player:say("You collected the smelly pollen into a bag.", TALKTYPE_POKEMON_SAY, false, player, item:getPosition())
